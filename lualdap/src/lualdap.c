...
static int lualdap_open(lua_State *L) {
	...
	const char *mech   = luaL_checkstring (L, 2);
	const char *param1 = luaL_optstring (L, 3, NULL);
	const char *param2 = luaL_optstring (L, 4, NULL);
	conn_data conn, *retval;
	int rc;
	struct berval bv={0, NULL}, *servercredp=NULL;
	...
	if (strcasecmp(mech, "SIMPLE") == 0) {
		bv.bv_val = param2;
		bv.bv_len = strlen(bv.bv_val);
		rc = ldap_sasl_bind_s(conn.ld, param1, NULL, &bv, NULL, NULL, NULL);
	}
	else {
		rc = ldap_sasl_bind_s(conn.ld, NULL, mech, NULL, NULL, NULL, &servercredp);
		if (rc == LDAP_SASL_BIND_IN_PROGRESS)
			rc = ldap_sasl_bind_s(conn.ld, NULL, mech, servercredp, NULL, NULL, &servercredp);
	}
exit:
	lua_pushnumber(L, rc);
