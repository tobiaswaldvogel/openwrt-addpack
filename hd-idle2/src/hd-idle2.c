 ...
 *
 * CVS Change hd_idle_log:
 * ---------------
 *
 * $hd_idle_log: hd-idle.c,v $
 ...
 * - Changed sleep time before calling sync after updating the hd_idle_log file to 1s
 *   (from 3s) to accumulate fewer dirty blocks before synching. It's still
 *   a compromize but the hd_idle_log file is for debugging purposes, anyway. A test
...
static void			spinup_disk_log (DISKSTATS *ds);
static char			*disk_name		(char *name);
static void			phex			(const void *p, int len,
									const char *fmt, ...);
static void			hd_idle_log		(int priority, const char* format, ...);
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "t:a:i:l:dh")) != -1) {
		...
		case 'h':
			printf("usage: hd-idle [-t <disk>] [-a <name>] [-i <idle_time>] [-d] [-h]\n");
			return(0);
	...
	for (;;) {
		...
		while (fgets(buf, sizeof(buf), fp) != NULL) {
			if (sscanf(buf, "%*d %*d %s %*u %*u %u %*u %*u %*u %u %*u %*u %*u %*u",
				...
				hd_idle_log(LOG_DEBUG, "probing %s: reads: %u, writes: %u",
					...
					*/
					for (it = it_root; it != NULL; it = it->next) {
						if (it->name == NULL || !strcmp(ds->name, it->name)) {
							...
							hd_idle_log(LOG_INFO, "disk %s: standby timeout %u seconds",
...
static void daemonize(void)
{
	...
	* access to the terminal */
	...
	/* write pid */
	if ((fp=fopen("/var/run/hd-idle.pid", "w")) != NULL)
	{
		fprintf(fp, "%d", getpid());
		fclose(fp);
	}
	...
	openlog("hd-idle", LOG_PID, LOG_DAEMON);
}
...
static void spindown_disk(char* name)
{
	...
	hd_idle_log(LOG_INFO, "disk %s: spindown", name);
	...
	if (ioctl(fd, SG_IO, &io_hdr) < 0) {
		...
		hd_idle_log(LOG_ERR, "error: SCSI command failed with status 0x%02x",
...
/* write a spin-up event message to the hd_idle_log file */
static void spinup_disk_log(DISKSTATS *ds)
{
hd_idle_log(LOG_INFO, "disk %s: spinup (running: %ld, stopped: %ld)",
			ds->name,
...
static char *disk_name(char *path)
{
	...
	*/
	...
	hd_idle_log(LOG_DEBUG, "using %s for %s", s, path);
	return(s);
}
...
static void phex(const void *p, int len, const char *fmt, ...)
{
	...
	while (len > 0) {
		...
		hd_idle_log(LOG_DEBUG, "%s", printbuf);
...
void hd_idle_log(int priority, const char* format, ...) {
	va_list va;
	char printbuf[1024];
